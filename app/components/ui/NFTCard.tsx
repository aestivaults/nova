...
export const NFTCard = memo(
  ({ nft, variant = "default", onBuy, onBid, className = "" }: cardprops) => {
    ...
    const shareUrl = useMemo(
      () => `${window.location.origin}/marketplace/${nft._id}`,
      [nft._id]
    );

    const formattedPrice = useMemo(
      () => formatEthPrice(isAuction ? nft.current_bid : nft.price),
      [isAuction, nft.current_bid, nft.price]
    );

    const formattedTimeRemaining = useMemo(
      () => (nft.auctionEndTime ? formatTimeRemaining(nft.auctionEndTime) : ""),
      [nft.auctionEndTime]
    );
              ...
              <div className="creator flex items-center md:space-x-2">
                <div className="relative w-5 h-5">
                  <Image
                    ...
                    className="c NFTCarobject-cover rounded-full"
                  />
                </div>
          ...
          <div className="mt-auto">
            <div className="flex justify-between items-center mb-3">
              <div>
                ...
                <p className="price text-base">{formattedPrice}</p>
              </div>
